(function (factory) {
  ...
  const get_cell_colors = (top_col, left_col, num, h_line, v_line, d_line, a_line) => {
  ...
  function get_pattern (rule_nums, num_of_cols, init_state, width, height) {
    const rule_fns = {
  ...
  const empty_cell_grid = (width, height) => {
  ...
  function ui (options, run, randomize_rules) {
    let ctrls = {
    ...
    let layout_folder = gui$$1.addFolder('Layout');
    ...
    let init_folder = gui$$1.addFolder('Initial State');
    init_folder
  ...
  let sketch = function (p) {
    ...
    function draw() {
      palette = get$1(options.palette_name);
      cell_dim =
